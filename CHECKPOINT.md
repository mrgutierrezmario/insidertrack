...
- **Dependabot round 2026-09-20 (evening)**: Actions bumps #1/#2/#4 merged; **#16 Python group** (fastapi 0.141, pydantic 2.13 — needed `email-validator` added) and **#17 frontend group** (React 19, Vite 8, TypeScript 7, jsdom 30 — needed Node 22 in CI and the image's UI stage) merged after being verified: type-check, build, 70 UI tests, and the candidate image run against a restored copy of the 2026-09-20 dump with every main route answering. #3/#5 (setup-python, setup-buildx) are waiting on Dependabot's rebase with auto-merge armed. **The live site is still on the old libraries** until the next `git pull && deploy/start.sh` — do that on a quiet day and check Admin → Data sources after. Repo setting "Allow auto-merge" enabled.
- **Live rebuild done 2026-09-20 18:25 ET** (`deploy/start.sh`): fastapi 0.141.1, pydantic 2.13.5, React 19 / Vite 8 bundle. 12 s of downtime; health ok, all sources ok, `errors_24h` 0, Funnel handlers intact, mcp untouched. Rollback point: `deploy/state/pre-rebuild-2026-09-20.sql.gz` + `git checkout v1.1.0 && deploy/start.sh`. Backup job re-registered from the renamed folder (operator ran `backup-setup.sh`; a bundle from 21:31 UTC exists).
- **Project #4 designed** (`../homelab-gitops/DESIGN.md`): k3s + Argo CD staging cluster running restored copies of the three apps; production stays on Compose. Not started.
- **Runbooks**: all three projects now document reboot recovery, manual backup and restore in the same shape (`deploy/OPERATIONS.md` each); the MCP's says it has nothing to back up.
...
- **First morning on the new dependencies** (same screens): scrapers ran, Admin → Data sources ok. Anything odd → rollback above.
- **"Did the MCP disturb the app?" — first observed day.** `/health` → `errors_24h.count` should be 0 and Admin → Data sources all OK with this morning's timestamps. A failing/stale source is most likely a government site changing, not the MCP; if it coincides with MCP calls in `docker compose logs mcp`, investigate timing. Two clean checks = record "MCP confirmed harmless" here. (MCP is at v0.2.0: 11 tools incl. fund leaderboard/track record.)

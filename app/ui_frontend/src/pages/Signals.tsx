...
function exportSignalsCSV(signals: SignalRow[]) {
  exportCSV(
    ["ticker", "label", "signal", "composite_score", "smart_money", "insider", "momentum", "sentiment", "rsi", "current_price"],
    signals.map((s) => {
      const sub = s.sub_scores || {};
      return [s.ticker, s.label, s.signal, s.composite_score ?? "", sub.smart_money ?? "", sub.insider ?? "", sub.momentum ?? "", sub.sentiment ?? "", s.rsi ?? "", s.current_price ?? ""];
    }),
            ...
            return (
              <div key={s.ticker} style={{
                ...
                <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginBottom: 10 }}>
                  <SubScore label="Smart Money" value={sub.smart_money} max={30} />
                  <SubScore label="Insider" value={sub.insider} max={25} />

...
#   3. wraps that in an encrypted remote called "stock-tracker-backup" so nothing
...
if [ "$(uname)" = Darwin ]; then
  PLIST=~/Library/LaunchAgents/com.mgnetwork.stock-tracker-backup.plist
  sed "s|__REPO__|$REPO|g; s|__HOME__|$HOME|g" mac/com.mgnetwork.stock-tracker-backup.plist > "$PLIST"
  launchctl unload "$PLIST" 2>/dev/null || true
  launchctl load "$PLIST"
  log "Scheduled nightly at 03:00 (launchd: com.mgnetwork.stock-tracker-backup). Log: deploy/state/backups/backup.log"
else
